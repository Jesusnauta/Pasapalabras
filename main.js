...
let rank = new Array();
const originalTimer = parseInt(
  document.getElementsByClassName("stopwatch")[0].innerHTML,
  10
);
...
const rankingShow = () => {
  if (ranking.findIndex(comparePoints) !== -1) {
    ranking.splice(ranking.findIndex(comparePoints), 0, {
      id: ranking.findIndex(comparePoints),
      rankedName: "Tu",
      points: points,
    });
  ...
  ranking.forEach((_, index) => {
    rank.push(
      `${ranking[index].id}º ${ranking[index].rankedName} - ${ranking[index].points} puntos.`
    );
  });

  document.getElementsByClassName(
    "gameOver"
  )[0].innerHTML = `Has obtenido ${points} aciertos y ${
    27 - points
  } fallos. Este es el ranking:`;
...
const showQuestion = (index, questionsToAnswer) => {
  ...
  document.getElementsByClassName("question")[0].innerHTML =
    questionsToAnswer[index].question;
};
...
const checkAnswer = (ind, questionToAnswer, answer) => {
  ...
  if (answer.toLowerCase() === questionToAnswer[ind].answer.toLowerCase()) {
    firstRound === true
      ? changeColor.classList.replace("neutral", "right")
      : changeColor.classList.replace("passed", "right");
    points++;
    ...
    firstRound === true
      ? changeColor.classList.replace("neutral", "wrong")
      : changeColor.classList.replace("passed", "wrong");
